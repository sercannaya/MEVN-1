...
methods: {
  async submit(user) {
    if (user.id) {
      await this.$axios.put("http://localhost:3000/users/" + user.id, {
        name: user.name,
        email: user.email,
      });
    } else {
      await this.$axios.post("http://localhost:3000/users/signup", {
        name: user.name,
        email: user.email,
    ...
    this.$store.commit(
      "users/storeData",
      (await this.$axios.get("http://localhost:3000/users")).data
    );
  },
  resetUser(user) {
